...
class Handler:
    def __init__(self, args, metadata,stop_flag, fetcher = 'cloudwatch', interval=30):
        ...
        if fetcher == 'cloudwatch':
            ...
            self.fetcher = CloudWatchFetcher(region_name=args.region, log_group=log_group_name)
        elif fetcher == 's3':
            self.fetcher = S3Fetcher(region_name=args.region, bucket_name=self.bucket_name)
        else:
            raise "Wrong Fetcher"
